...
{	
	for(var i= 0; i < document.getElementsByName("echoShow_linkFile[]").length; i++)
	{
		if (document.getElementsByName("echoShow_linkFile[]")[i].value != "")
		{
			if (document.getElementsByName("echoShow_linkTitle[]")[i].value == "")
			{
				document.getElementsByName("echoShow_linkTitle[]")[i].up('li').addClassName('error');
				 return $I18N.MISSING;
			}
			
			if (document.getElementsByName("echoShow_linkCredits[]")[i].value == "")
			{
				document.getElementsByName("echoShow_linkCredits[]")[i].up('li').addClassName('error');
				 return $I18N.MISSING;
			}
			return null;
		}
...
addEchoShow = function(url, idShow, obj)
{
    var ajax = new Ajax.Request(url, {
                onSuccess: function(transport) {
                    if (idShow)
                    {
                        $(idShow).insert({bottom: transport.responseText});
...
removeEchoShow = function(button, id)
{
	if($$('#echoShow .collapsable').length > 1)
	{
