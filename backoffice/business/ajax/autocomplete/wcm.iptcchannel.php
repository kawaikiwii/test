...
function generateListe($debut,$liste,$lang) 
{
    $MAX_RETURN = 12;
    $i = 0;
    foreach ($liste as $key=>$val) 
    {
        if ($i<$MAX_RETURN && (substr(strtolower($val[$lang]), 0, strlen($debut))==$debut || substr(strtolower($key), 0, strlen($debut))==$debut)) 
        {
            echo '<li id="'.$key.'" style="display: block; -moz-border-radius: 3px;padding: 0 100px 0 5px;cursor: pointer;margin-top: 2px;">';
			echo getConst($val[$lang]).":".$key;
			echo '</li>';
            $i++;
        }
    }
...
generateListe($prefix,$data,$lang);
