...
function generateListe($prefix, $max, $lang, $countryCode) {
	...
	$strQueryCountryCode = "";
    if($countryCode != "") {
		$strQueryCountryCode = " AND countryCode = '".$countryCode."'";
    }
    if (!$enum->beginEnum("name like '$prefix%'".$strQueryCountryCode, "name ASC LIMIT 0, $max"))
        return null;
    ...
    while ($enum->nextEnum()) {
    	...
    	if($enum->admin1 != "") {
		    ...
		    if (!$obj1->beginEnum("ISO = '".$enum->countryCode."'", "country ASC LIMIT 0, 1"))
		        return null;
			while ($obj1->nextEnum()) {
				$country = $obj1->getAlternateName($lang);
				if($country == "") $country = $obj1->country;
				$countryName .= $country;
			}
			unset ($obj1);
    	}
        ...
        echo '<li id="'.$enum->geonameId.'" style="display: block; -moz-border-radius: 3px;padding: 0 100px 0 5px;cursor: pointer;margin-top: 2px;" title="'.$enum->name.' ('.$countryName.$admin1.$admin2.')" onClick="updateCity(this.id, \''.$enum->name.'\');">';
		$cityName = $enum->getAlternateName($lang);
		if($cityName == "") $cityName = $enum->name;
        echo $cityName.' ('.$countryName.$admin1.$admin2.')';
        echo '</li>';
    }
    $enum->endEnum();
