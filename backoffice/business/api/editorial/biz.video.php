...
class video extends bizobject {
    ...
    {	
        ...
        else
        {
            ...
            if (parent::save($source))
            {
                ...
                {
                	//ne pas créer de notice pour les vidéos extraites des champs embed des news
                	if (empty($this->sourceId))
                		$this->createNoticeForMedia();

                	// ne pas dupliquer les vidéos extraites des champs embed des news
                	if ($this->workflowState == "published" && empty($this->sourceId))
                    {
    ...
    /**
     * Refresh object from unique sourceId
     *
     * @param int sourceId used to refresh 
     *
     * @return the video object
     */
	public function refreshBySourceId($sourceId) {
        $sql = 'SELECT id FROM '.$this->getTableName().' WHERE sourceId=?';
        $id = $this->database->executeScalar($sql, array($sourceId));
        return $this->refresh($id);
    }
    
	static function getVideoFormatsFromConfig() 
	{
        $config = wcmConfig::getInstance();
        $formats = explode(",", $config['afprelax.videoFormats.formats']);
        ksort($formats);
        return $formats;
    }
