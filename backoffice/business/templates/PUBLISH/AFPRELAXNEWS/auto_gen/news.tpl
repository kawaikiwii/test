{load class="news" object="onews" where="id = `$context.id`"}
...
{capture name="mobileList"}
	{if $news.import_feed == "icm" }
		{include file="`$config.wcm.templates.path`PUBLISH/AFPRELAXNEWS/items/news/news.mobile.list.tpl" bizobject="$news" obizobject="$onews"}
	{/if}
{/capture}
{dump file=$filename|replace:'%format%':'mobile.list' content=$smarty.capture.mobileList utf8=true}
