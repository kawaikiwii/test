...
{/foreach}
	<div class="ari-desk-header ari-desk-{$rootChannel->css}">
		<h2 class="ari-desk-header-title"><a href="#" onclick="ARe.search.rubric(this, 'slideshow', '{$rootChanIds}')">{$rootChannel->title}</a></h2>
		<div class="ari-desk-header-category">
{foreach from=$channels item=channel name=foo}
{assign var=dex value=$channel->id}
				<a href="#" onclick="ARe.search.rubric(this, 'slideshow', '{','|implode:$bChannels.$dex}')">{$channel->title}</a>{if !$smarty.foreach.foo.last},{/if} 
{/foreach}
    	</div>
	</div>
...
{if $cDate == ''}
						<h1 id="ari-separator-slideshow-{$rootChannel->css}" class="ari-desk-separator-date">{$nDate}</h1>
{else}
						<h1 class="ari-desk-separator-date">{$nDate}</h1>
{/if}
{assign var="cDate" value=$nDate}
{/if}
{include file=$filename|replace:'%format%':'list'}
{/if}
{/foreach}
	</div>
{/capture}
{assign var="filename" value="`$config.wcm.webSite.repository`sites/`$site->code`/homes/slideshow.`$rootChannel->css`.html"}
