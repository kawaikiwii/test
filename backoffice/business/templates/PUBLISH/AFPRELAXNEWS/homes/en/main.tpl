...
{assign var="slideshowURL" value=$slideshowURL|replace:'%format%.html':'detail.xml'}
						<div class="ari-section-title mid"><span class="ari-more"><a href="#" onclick=" ARe.search.type('Slide shows', 'slideshow')" title="Browse news">more</a></span><span class="ari-title">RELAX SLIDESHOWS</span></div>
...
{assign var="videoIllustration" value="http://video.relaxnews.com/uploads/video_afp.jpg"}
...
{assign var="videoEmbed" value=$videoEmbed|replace:'width=':'width="278" oldW='}
				...
				<div class="left">
					<div class="ari-section essential">
						<div class="ari-section-title mid"><div class="ari-more"><a href="#" onclick=" ARe.search.type('News', 'news')" title="Browse news">more</a></div><span class="ari-title">RELAX ESSENTIAL</span></div>
						<div class="ari-content">
...
{/foreach}
				...
				<div class="ari-section agenda">
						<div class="ari-section-title mid"><span class="ari-more"><a href="#" onclick=" ARe.search.type('Events', 'event')" title="Browse events">more</a></span><span class="ari-title">RELAX EVENTS</span></div>
						<div class="ari-content">
							<div class="ari-event ari-next-week">
								<h4 class="ari-section-subtitle  ari-entertainment">Next week</h4>
								<div class="ari-item" onclick="ARe.pview('event', {$EVENT_NEXT_WEEK->id});">
...
{/if}	
{/foreach}	
									<p>{$EVENT_NEXT_WEEK->startDate|date_format:"%Y-%m-%d"} - {$EVENT_NEXT_WEEK->endDate|date_format:"%Y-%m-%d"}</p>
								</div>
							</div>
							<div class="ari-event ari-next-month">
								<h4 class="ari-section-subtitle  ari-entertainment">Next month</h4>
...
{/if}	
{/foreach}
									<p>{$EVENT_NEXT_MONTH->startDate|date_format:"%Y-%m-%d"} - {$EVENT_NEXT_MONTH->endDate|date_format:"%Y-%m-%d"}</p>
								</div>
							</div>
							<div class="ari-event ari-later">
								<h4 class="ari-section-subtitle  ari-tourism">And after ...</h4>
...
{/if}	
{/foreach}
									<p>{$EVENT_LAST->startDate|date_format:"%Y-%m-%d"} - {$EVENT_LAST->endDate|date_format:"%Y-%m-%d"}</p>
								</div>
							</div>
						</div>			
					</div>
				...
				<div class="center">
					<div class="ari-section tagcloud">
						<div class="ari-section-title mid"><span class="ari-title">RELAX KEYWORDS</span></div>
						<div class="ari-content">
							<p class="ari-tagcloud-click">click a term for more</p>
							<div class="ari-animation">
...
{if $smarty.foreach.foo.index  == 0}{assign var=sectionTitle value="Luxury"}{assign var=sectionId value="128"}{/if}
{if $smarty.foreach.foo.index  == 1}{assign var=sectionTitle value="Products"}{assign var=sectionId value="511"}{/if}
{if $smarty.foreach.foo.index  == 2}{assign var=sectionTitle value="People"}{assign var=sectionId value="141"}{/if}
{if $smarty.foreach.foo.index  == 3}{assign var=sectionTitle value="Trends"}{assign var=sectionId value="512"}{/if}
...
{/foreach}
		...
		<div class="bottom">
			<div class="ari-section">
				<div class="ari-section-title full"><span class="ari-title">RELAX UNIVERSE</span></div>
				<div class="ari-content">
					<div class="ari-sub-section">
						<h2 class="ari-sub-title">Relax rubric :</h2>
						<p>
...
{/foreach}				
					...
					<div class="ari-sub-section">
						<h2 class="ari-sub-title">Relax selection : </h2>
						<p>
...
{/foreach}	
					...
					<div class="ari-sub-section">
						<h2 class="ari-sub-title">Relax contact : </h2>
						<p>
	<a href="mailto:managing.editor@afprelaxnews.com">La rédaction</a>, <a href="mailto:contact@afprelaxnews.com">le commercial</a>, <a href="mailto:support.tech@afprelaxnews.com">le technique						</p>
					</div>
				</div>
			</div>
		</div>
		<div class="footer">
			<p>&copy; AFP/RELAXNEWS 2009-{$smarty.now|date_format:"%Y"}</p>
			<p style="font-size:0.65em;">Updated : {$smarty.now|date_format:"%r"}</p>
		</div>
{/capture}
