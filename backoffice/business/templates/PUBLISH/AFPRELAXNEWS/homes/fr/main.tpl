...
{assign var="videoEmbed" value=$videoEmbed|replace:'width=':'width="278" oldW='}
					<div class="ari-section video">
						<div class="ari-section-title video"><span class="ari-more"><a href="#" onclick=" ARe.search.type('Videos', 'video')" title="Browse videos">more</a></span><span class="ari-title">RELAX VIDÉOS</span></div>
						<div class="ari-content">
							<div class="ari-item focused">
...
{/foreach}
				...
				<div class="ari-section agenda">
						<div class="ari-section-title mid"><span class="ari-more"><a href="#" onclick=" ARe.search.type('Events', 'event')" title="Browse events">more</a></span><span class="ari-title">RELAX ÉVÉNEMENTS</span></div>
						<div class="ari-content">
...
{/if}	
{/foreach}	
									<p>{$EVENT_NEXT_WEEK->startDate|date_format:"%d-%m-%Y"} - {$EVENT_NEXT_WEEK->endDate|date_format:"%d-%m-%Y"}</p>
								</div>
							</div>
...
{/if}	
{/foreach}
									<p>{$EVENT_NEXT_MONTH->startDate|date_format:"%d-%m-%Y"} - {$EVENT_NEXT_MONTH->endDate|date_format:"%d-%m-%Y"}</p>
								</div>
							</div>
...
{/if}	
{/foreach}
									<p>{$EVENT_LAST->startDate|date_format:"%d-%m-%Y"} - {$EVENT_LAST->endDate|date_format:"%d-%m-%Y"}</p>
								</div>
							</div>
						</div>			
					</div>
				...
				<div class="center">
					<div class="ari-section tagcloud">
						...
						<div class="ari-content">
							<p class="ari-tagcloud-click">cliquer sur un terme</p>
							<div class="ari-animation">
...
{if $smarty.foreach.foo.index  == 0}{assign var=sectionTitle value="Luxe"}{assign var=sectionId value="134"}{/if}
{if $smarty.foreach.foo.index  == 1}{assign var=sectionTitle value="Produits"}{assign var=sectionId value="509"}{/if}
{if $smarty.foreach.foo.index  == 2}{assign var=sectionTitle value="People"}{assign var=sectionId value="142"}{/if}
{if $smarty.foreach.foo.index  == 3}{assign var=sectionTitle value="Tendances"}{assign var=sectionId value="510"}{/if}
	<div id="home-thema-people" class="ari-section">
			<div class="ari-section-title small"><span class="ari-sub-title" onclick="ARe.folder.open('{$sectionTitle|escape:"quotes"}', {$sectionId})">{$sectionTitle}</span></div>
			<div class="ari-content">
				<ul class="ari-list-item">
...
{/foreach}	
					...
					<div class="ari-sub-section">
						...
						<p>
							<a href="mailto:managing.editor@afprelaxnews.com">La rédaction</a>, <a href="mailto:contact@afprelaxnews.com">le commercial</a>, <a href="mailto:support.tech@afprelaxnews.com">le technique</a>
						</p>
					</div>
				</div>
			</div>
