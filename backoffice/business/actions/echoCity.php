...
class echoCityAction extends wcmMVC_BizAction
{
	...
	/**
     *  fonction permettant d'uploader un fichier en créeant une forme compatible du nom  
     *
     * @return string nom du fichier nettoyé
     * @param string $ville (nom de la ville)
     * @param string $titre (titre du fichier)
     * @param string $fichier (chemin du fichier)
     * @param string $prefix (prefix spécifique)
     * @param string $getkey (utilisé pour les variables de type tableau)
     */
    private function makeCompatiblePhoto($ville, $titre, $fichier, $prefix, $getkey = null)
    {
        ...
        if (isset($getkey))
        {
        	$initialName = $fichier["name"][$getkey];
        	$tempName = $fichier["tmp_name"][$getkey];
        }
        else 
        {
        	$initialName = $fichier["name"];
        	$tempName = $fichier["tmp_name"];
        }
           
        //[prefixe]_[un bout du nom de ville]-[chaine de caractère libre].jpg
        $ville = relaxGUI::clear_str($ville);
     	$titre = relaxGUI::clear_str($titre);
     	
     	$extension=pathinfo($initialName,PATHINFO_EXTENSION);
     	$newName = $prefix."_".$ville."-".$titre.".".$extension;
        
        $finalPathName = relaxGUI::availableNameBeforeUpload($photoPath.DIRECTORY_SEPARATOR.$newName);
        $finalName = basename($finalPathName);
        
        if (move_uploaded_file($tempName, $finalPathName))
			return $finalName;
		else		
			return false;
	...
	private function saveInserts()
	{
		...
		$titleMapCityFile = getArrayParameter($_REQUEST, 'mapCityCaption');
		if (empty($titleMapCityFile) && isset($this->context->mapCityCaption)) $titleMapCityFile = $this->context->mapCityCaption;
		
		$titleMapTransportCaption = getArrayParameter($_REQUEST, 'mapTransportCaption');
		if (empty($titleMapTransportCaption) && isset($this->context->mapTransportCaption)) $titleMapTransportCaption = $this->context->mapTransportCaption;
				
		$photoPath = WCM_DIR.DIRECTORY_SEPARATOR.$config['wcm.backOffice.photosPathLesEchos'];
		
		if (isset($_FILES["mapCityFile"]) && !empty($_FILES["mapCityFile"]))	
		{	
			 $checkname = $this->makeCompatiblePhoto($mainCity, $titleMapCityFile, $_FILES["mapCityFile"], "info");
			 if ($checkname) $_REQUEST["mapCityFile"] = $checkname;
		}		
		
		if (isset($_FILES["mapTransportFile"]) && !empty($_FILES["mapTransportFile"]))	
		{	
			 $checkname = $this->makeCompatiblePhoto($mainCity, $titleMapTransportCaption, $_FILES["mapTransportFile"], "info");
			 if ($checkname) $_REQUEST["mapTransportFile"] = $checkname;
		}
		...
		if(isset($_REQUEST['echoAirport_title'])) 
		{
			...
			if(is_array($echoAirportTitle))
			{
				...
				{		
					if (!empty($_FILES["echoAirport_mapFile"]["tmp_name"][$key]))	
					{	
						 $checkname = $this->makeCompatiblePhoto($mainCity, getArrayParameter($echoAirportMapTitle, $key), $_FILES["echoAirport_mapFile"], "airports", $key);
						 if ($checkname) $echoAirportMapFile[$key] = $checkname;
						 else $echoAirportMapFile[$key] = $echoAirportMapFile2[$key];
					}
					else 
		...
		if(isset($_REQUEST['echoSlideshow_title'])) 
		{
			...
			if(is_array($echoSlideshowTitle))
			{
				...
				{
					if (!empty($_FILES["echoSlideshow_file"]["tmp_name"][$key]))	
					{	
						 $checkname = $this->makeCompatiblePhoto($mainCity, getArrayParameter($echoSlideshowTitle, $key), $_FILES["echoSlideshow_file"], "slideshow", $key);
						 if ($checkname) $echoSlideshowFile[$key] = $checkname;
						 else $echoSlideshowFile[$key] = $echoSlideshowFile2[$key];
					}
					else 
		...
		if(isset($_REQUEST['echoEvent_title'])) 
		{
			...
			if(is_array($echoEventTitle))
			{
				...
				{
					if (!empty($_FILES["echoEvent_linkFile"]["tmp_name"][$key]))	
					{	
						 $checkname = $this->makeCompatiblePhoto($mainCity, getArrayParameter($echoEventLinkTitle, $key), $_FILES["echoEvent_linkFile"], "events", $key);
						 if ($checkname) $echoEventLinkFile[$key] = $checkname;
						 else $echoEventLinkFile[$key] = $echoEventLinkFile2[$key];
					}
					else 
		...
		if(isset($_REQUEST['echoStation_title'])) 
		{
			...
			if(is_array($echoStationTitle))
			{
				...
				{	
					if (!empty($_FILES["echoStation_mapFile"]["tmp_name"][$key]))	
					{	
						 $checkname = $this->makeCompatiblePhoto($mainCity, getArrayParameter($echoStationMapTitle, $key), $_FILES["echoStation_mapFile"], "stations", $key);
						 if ($checkname) $echoStationMapFile[$key] = $checkname;
						 else $echoStationMapFile[$key] = $echoStationMapFile2[$key];
					}
					else 
		...
		if(isset($_REQUEST['echoShow_title'])) 
		{
			...
			if(is_array($echoShowTitle))
			{
				...
				{
					if (!empty($_FILES["echoShow_linkFile"]["tmp_name"][$key]))	
					{	
						 $checkname = $this->makeCompatiblePhoto($mainCity, getArrayParameter($echoShowLinkTitle, $key), $_FILES["echoShow_linkFile"], "show", $key);
						 if ($checkname) $echoShowLinkFile[$key] = $checkname;
						 else $echoShowLinkFile[$key] = $echoShowLinkFile2[$key];
					}
					else 
