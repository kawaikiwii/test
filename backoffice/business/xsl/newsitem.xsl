...
<xsl:stylesheet version="1.0"
  ...
  <!-- Template for rendering newsitem -->
  <xsl:template match="newsitem">
    <table cellspacing="0" cellpadding="0" border="0" width="90%">
      <tr>
        <td width="20" align="center">
          <img src="img/icons/news.gif" alt="" width="16" height="16" border="0" hspace="2">
            <xsl:if test="$callback != ''">
              <xsl:attribute name="onclick"><xsl:value-of select="$callback"/>('newsitem',<xsl:value-of select="id"/>, '<xsl:call-template name="formatText"><xsl:with-param name="text" select="title"/></xsl:call-template>')</xsl:attribute>
            </xsl:if>
           </img>
        </td>
        <td width="100%">
          <div class="item" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
            ...
            <xsl:value-of select="php:function('constant', string('_BIZ_NEWSITEM'))"/> (<xsl:value-of select="id"/>): <strong><xsl:apply-templates select="title"/></strong>
        ...
        <td align="center">
          <xsl:if test="text != ''">
            <img src="img/texte_long.gif" border="0" style="cursor:pointer" hspace="2">
              ...
              <xsl:attribute name="id">swapImage_newsitem_<xsl:value-of select="id"/></xsl:attribute>
              <xsl:attribute name="onclick">swap('ne_<xsl:value-of select="id"/>', this, 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
            </img>
          </xsl:if>
        </td>
        <td>
          <div class="item">
            ...
            <xsl:if test="text != ''">
                  <xsl:attribute name="onclick">swap('ne_<xsl:value-of select="id"/>', 'swapImage_newsitem_<xsl:value-of select="id"/>', 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
                </xsl:if>
              <xsl:if test="createdAt != ''">
                <xsl:value-of select="php:function('constant', string('_BIZ_CREATED_AT'))"/>: <xsl:value-of select="createdAt"/>
              </xsl:if>
              <xsl:if test="source != ''">
                <![CDATA[ ]]><xsl:value-of select="php:function('constant', string('_BIZ_BY'))"/>: <xsl:value-of select="source"/>
              </xsl:if>
            </div>
            <xsl:if test="text != ''">
              <div style="display:none; margin: 0; padding: 0 0 8px;" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
                <xsl:attribute name="id">ne_<xsl:value-of select="id"/></xsl:attribute>
                <em>
