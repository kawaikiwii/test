...
<xsl:stylesheet version="1.0"
  ...
  <!-- Template for rendering channels -->
    ...
    <xsl:choose>
      <xsl:when test="$mode = 'tree'">
        <table cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <img src="img/icons/rubrique.gif" alt="" width="16" height="16" border="0" hspace="2"/>
            </td>
            <td>
              <div class="item" onMouseOver="this.className='itemOver';" onMouseOut="this.className='item';">
                <xsl:if test="$callback != ''">
                  <xsl:attribute name="onclick"><xsl:value-of select="$callback"/>('channel',<xsl:value-of select="id"/>,'<xsl:call-template name="formatText"><xsl:with-param name="text" select="title"/></xsl:call-template>')</xsl:attribute>
                </xsl:if>
                <strong>
          ...
          <tr>
            ...
            <td width="100%">
              <div class="item" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
                ...
                <xsl:value-of select="php:function('constant', string('_BIZ_CHANNEL'))"/> (<xsl:value-of select="id"/>): <strong>
            ...
            <td align="center">
              <xsl:if test="description != ''">
                <img src="img/texte_long.gif" border="0" style="cursor:pointer" hspace="2">
                  ...
                  <xsl:attribute name="id">swapImage_channel_<xsl:value-of select="id"/></xsl:attribute>
                  <xsl:attribute name="onclick">swap('ch_<xsl:value-of select="id"/>', this, 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
                </img>
              </xsl:if>
            </td>
            <td>
              <div class="item">
                <xsl:if test="description != ''">
                  <xsl:attribute name="onclick">swap('ch_<xsl:value-of select="id"/>', 'swapImage_channel_<xsl:value-of select="id"/>', 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
                </xsl:if>
                ...
                <xsl:if test="description != ''">
                  <div style="display:none; margin: 0; padding: 0 0 8px;" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
                    <xsl:attribute name="id">ch_<xsl:value-of select="id"/></xsl:attribute>
                    <em>
