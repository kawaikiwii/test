...
<xsl:stylesheet version="1.0"
  ...
  <!-- Template for rendering forum -->
  <xsl:template match="forum">
    <table cellspacing="0" cellpadding="0" border="0" width="90%">
      <tr>
        <td width="20" align="center">
          <img src="img/icons/forum.gif" alt="" width="16" height="16" border="0" hspace="2">
            <xsl:if test="$callback != ''">
              <xsl:attribute name="onclick"><xsl:value-of select="$callback"/>('forum',<xsl:value-of select="id"/>, '<xsl:call-template name="formatText"><xsl:with-param name="text" select="title"/></xsl:call-template>')</xsl:attribute>
            </xsl:if>
          </img>
        </td>
        <td width="100%">
          <div class="item" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
            ...
            <xsl:value-of select="php:function('constant', string('_BIZ_FORUM'))"/> (<xsl:value-of select="id"/>): <strong><xsl:apply-templates select="title"/></strong>
        ...
        <td align="center">
          <xsl:if test="description != ''">
            <img src="img/texte_long.gif" border="0" style="cursor:pointer" hspace="2">
              ...
              <xsl:attribute name="id">swapImage_forum_<xsl:value-of select="id"/></xsl:attribute>
              <xsl:attribute name="onclick">swap('fo_<xsl:value-of select="id"/>', this, 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
            </img>
          </xsl:if>
        </td>
        <td>
          <div class="item">
            ...
            <xsl:if test="description != ''">
              <xsl:attribute name="onclick">swap('fo_<xsl:value-of select="id"/>', 'swapImage_forum_<xsl:value-of select="id"/>', 'img/texte_long.gif', 'img/texte_court.gif');</xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="description != ''">
              <div style="display:none; margin: 0; padding: 0 0 8px;" onmouseover="this.className='itemOver';" onmouseout="this.className='item';">
                <xsl:attribute name="id">fo_<xsl:value-of select="id"/></xsl:attribute>
                <em>
