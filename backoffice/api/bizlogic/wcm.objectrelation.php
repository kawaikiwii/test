 ...
 *
 */
class wcmObjectrelation extends wcmObject
{
    //
	...
	public static function getRelationsToBizobject($bizobject, $kind=null, $sourceClass=null, $toXML = false)
    {
        $where = "destinationClass='" . $bizobject->getClass() . "' AND destinationId=" . $bizobject->id;
        if ($sourceClass)
        {
            $where .= " AND sourceClass='" . $sourceClass . "'";
        }
        if ($kind)
        {
            $where .= " AND kind=" . $kind;
        ...
        $relations = array();
        $relation = new wcmBizrelation();
        $relation->beginEnum($where, "sourceClass,validityDate, rank");
        while ($relation->nextEnum())
        {
            // clone before retrieve getAssocArray
            $rel = clone($relation);
   ...
   public function copy($fromDate, $toDate)
   {
	...
	/* OPTIMISATION NSTEIN 01/07/2009
