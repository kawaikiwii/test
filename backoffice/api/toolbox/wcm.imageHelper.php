...
/**
 * Generic tool to manage images file (Allow to resize, crop, etc.... ? )
 */
class wcmImageHelper 
{
	...
	public function __construct($fileName)
	{
		...
		{
		    wcmProject::getInstance()->logger->logError(_FILENOTFOUND_EXCEPTION . ': ' . $fileName);
		    return;
		}
	...
	/**
	 *	Create a square crop image
	 *
     *  @param String	$destination	Name of the output file
     *  @param Int		$formatValue	Width and height of the square
     *  @param Int		$type			Image output format (Default : IMG_JPG)
	 */
	public function squareCrop($destination, $formatValue, $type = IMG_JPG)
	{
		$w = $this->img_size[0];
		$h = $this->img_size[1];
			
		if ($this->gd2)
			$destsrc = imagecreatetruecolor($formatValue,$formatValue);				
		else
			$destsrc = imagecreate($formatValue,$formatValue);	
		...
		{
			// cas du portrait
			imagecopy($destsrc, $this->img_src, 0, 0, 0, 0, $w, $h);
		}
		else
		{
			// cas du paysage - image centrée
			$tlx = floor($w / 2) - floor ($formatValue / 2);
			$tly = floor($h / 2) - floor($formatValue / 2);
			imagecopy($destsrc, $this->img_src, 0, 0, $tlx, $tly, $w, $h);
		}
