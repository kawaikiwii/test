...
class wcmFormGUI
{
    ...
    static function renderRadiosFieldWithKey($name, array $options, $value = null, $label = null, $display = null, array $exclude = null)
    {
        echo '<nobr>';
		if ($label) echo '<label style="float: left;width: 150px;">'.textH8($label).'</label>';
		...
		foreach($options as $key=>$val)
        {
        	...
        	if (empty($exclude) || in_array($key,$exclude)== false/*!= $key*/)
        	{
	        	$check = "";
	...
	static function renderCommonListField($name, $values = null, $attributes = null, $acOptions = null)
    {
        ...
        if (is_array($tab))
        {
            foreach ($tab as $value)
            {
                if (trim($value) != '')
                {
                    echo '<li'.$css_clear.'  id="'.$value.'"><a href="#" onclick="_wcmDeleteElement($(this).up(), \'' . $name . '\'); return false;"><span>' . _DELETE . '</span></a> <em>' . $value . '</em></li>';
                }
            }
        } 
        echo '</ul>';
