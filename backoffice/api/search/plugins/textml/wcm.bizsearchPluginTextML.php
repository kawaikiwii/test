...
class wcmBizsearchPluginTextML extends wcmBizsearchPluginAbstract
{
    ...
    public function initSearch($searchId, $query, $sortingCriteria = null, $source = "BO")
    {
        ...
        // Initialize query
        $result = $this->initQuery($this->queryString, $this->parameters, null, $source);
        if ($result === null)
        {
            return null;
        }
     ...
     */
    protected function initQuery($queryString = null, $parameters = null, $includes = null, $source = "BO")
    {
        // Convert the query string or parameters into a native query
        if($source == "BO")
        	$result = parent::initQueryBO($queryString, $parameters);
        else
        	$result = parent::initQueryFO($queryString, $parameters);
        if ($result === null)
            return null;
    ...
    protected function convertQuery_Boolean($query, $context = null)
    {
        ...
        // Build native query from sub-queries
        $nativeQuery = $this->buildNativeBooleanQuery($subQueries);
    ...
    protected function convertQuery_MultiTerm($query, $context = null)
    {
        ...
        // Build native term from terms
        $nativeTerm = $this->buildNativeBooleanQuery($terms);
    ...
    protected function convertTermText($field, $text, $context = null)
    {
        ...
        else
        {
            ...
            case 'StringIndex':
                ...
                break;
            case 'DateTimeIndex':
                $indexValue = preg_replace(array('/T(\d+)x(\d+)x(\d+)/i','/[mx]([0-9]+)/i'), array('T$1:$2:$3','-$1'), $indexValue);
                ...
                if(strpos($indexValue, '..') !== false)
                {
                    ...
                    // Convert the index value according to the kind of index
                    if ($parts[0] && !is_numeric($parts[0])) $parts[0] = strtotime($parts[0]);
                    if ($parts[1] && !is_numeric($parts[1])) $parts[1] = strtotime($parts[1]);

                    $nativeText .= wcmTextmlQueryHelper::generateIntervalDateTimeQuery($indexName, $parts[0], $parts[1]);
                }
                elseif (!is_numeric($indexValue))
                {
                    $nativeText .= wcmTextmlQueryHelper::generateDateTimeQuery($indexName, strtotime($indexValue));
                }
                elseif (intval($indexValue) <= 365)
                {
                    $nativeText .= wcmTextmlQueryHelper::generateRelativeDateTimeQuery($indexName, intval($indexValue));
                }
                else
                {
                    $nativeText .= wcmTextmlQueryHelper::generateDateTimeQuery($indexName, $indexValue);
                } 
                break;
            case 'DateIndex':
    ...
    protected function convertTermField($field, $nativeText, $context = null)
    {
        ...
        if('DateTimeIndex' === $this->textmlServer->indexList[$indexName]['kind'])
            return $nativeText;
        else
    ...
    /**
     * Builds a native TextML query from optional, required, and prohibited sub-queries as generated
     * with the convertQuery_BooleanHelper and convertQuery_MultiTermHelper methods.
     *
     * @param array Assoc. array of optional, required, and prohibited sub-queries
     *
     * @return string The corresponding native query
     */
    protected function buildNativeBooleanQuery($subQueries)
    {
        // Group optional (no-sign) sub-queries
        $optionalSubQueries = $subQueries['optional'];
        $optionalQuery = implode('', $optionalSubQueries);

        // Use an <orkey> wrapper only if we have MORE THAN ONE optional sub-query:
        //
        // a b [...]
        //
        // <orkey>
        //     a <!-- optional sub-query 1 -->
        //     b <!-- optional sub-query 2 -->
        //     [...]
        // </orkey>
        //
        $optionalOrKey = (count($optionalSubQueries) > 1);

        // Group required (+) sub-queries
        $requiredSubQueries = $subQueries['required'];
        $requiredQuery = implode('', $requiredSubQueries);

        // Use an <andkey> wrapper only if we have BOTH an optional sub-query (<key> or <orkey>
        // element) and AT LEAST ONE required sub-query:
        //
        // a +b [...]
        //
        // <andkey>
        //     a <!-- optional sub-query (<key> or <orkey> element) -->
        //     b <!-- required sub-query 1 -->
        //     [...]
        // </andkey>
        //
        // OR we have MORE THAN ONE required sub-query:
        //
        // +a +b [...]
        //
        // <andkey>
        //     a <!-- required sub-query 1 -->
        //     b <!-- required sub-query 2 -->
        //     [...]
        // </andkey>
        //
        $requiredAndKey = ($optionalQuery && $requiredQuery
                           || count($requiredSubQueries) > 1);

        // Group prohibited (-) queries
        $prohibitedSubQueries = $subQueries['prohibited'];
        $prohibitedQuery = implode('', $prohibitedSubQueries);

        // Use an <andnotkey> wrapper only if we have AT LEAST ONE prohibited sub-query:
        //
        // a -b [...]
        //
        // <andnotkey>
        //     a <!-- first child; allowed sub-query (<key>, <property>, <andkey> or <orkey> element) -->
        //     b <!-- other children; prohibited sub-query 1 -->
        //     [...]
        // </andnotkey>
        //
        $prohibitedAndNotKey = (count($prohibitedSubQueries) > 0);

        // Build the native query by nesting sub-queries thus:
        //
        // [<andnotkey>]
        //     [<andkey>]
        //         [<orkey>]
        ...
        //     [</andkey>]
        //     <!-- prohibited sub-queries -->
        // [</andnotkey>]
        //
        $nativeQuery = '';

        if ($prohibitedAndNotKey) $nativeQuery .= '<andnotkey>';
        if ($requiredAndKey)      $nativeQuery .= '<andkey>';
        if ($optionalOrKey)       $nativeQuery .= '<orkey>';
        if ($optionalQuery)       $nativeQuery .= $optionalQuery;
        if ($optionalOrKey)       $nativeQuery .= '</orkey>';
        if ($requiredQuery)       $nativeQuery .= $requiredQuery;
        if ($requiredAndKey)      $nativeQuery .= '</andkey>';
        if ($prohibitedQuery)
        {
            // If there are no optional and required sub-queries:
            //
            // -a [...]  <==>  *:* -a [...]
            //
            // Then we substitute the following property to match anything for the first child
            // (allowed sub-query):
            //
            // <property NAME="Collection"><elem><anystr/></elem></property>
            //
            if (!$optionalQuery && !$requiredQuery)
                $nativeQuery .= '<property NAME="Collection"><elem>/<anystr />/</elem></property>';
    ...
    public function getDateTimeIndexList()
    {
        ...
        if(null === $dateIndexes)
        {
            ...
            foreach($this->getIndexList() as $indexName => $indexInfos)
            {
                if($indexInfos['kind'] === 'DateTimeIndex')
                {
                    $dateIndexes[] = $indexName;
                }
            }
