...
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode,partnerfeeds){
 this.htmlNode="";
 this.acolor="";
 this.scolor="";
 this.tr=0;
 ...
 this.partnerfeeds=partnerfeeds;
 ...
 dhtmlXTreeObject.prototype._drawNewTr=function(htmlObject)
{
 ...
 dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,partnerfeeds){
 ...
 if(((parentObject.XMLload==0)&&(this.XMLsource))&&(!this.XMLloadingWarning))
{
 ...
 Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,itemActionHandler,1,partnerfeeds);
 ...
 if(parentObject.childsCount>=2)
{
 ...
 dhtmlXTreeObject.prototype.insertNewItem=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,partnerfeeds){
 var parentObject=this._globalIdStorageFind(parentId);
 if(!parentObject)return(-1);
 return this._attachChildNode(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,null,partnerfeeds);
};
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
{
 ...
 var partnerfeeds=node.childNodes[i].getAttribute("partnerfeeds");
 ...
 temp.XMLload=1;
 dhtmlObject.insertNewItem(parentId,cId,name,0,im0,im1,im2,zST,chd,partnerfeeds);
...
dhtmlXTreeObject.prototype._alertPartnerFedds=function(dhtmlObject){
	if(dhtmlObject.parentObject.id != this.rootId)
		this._alertPartnerFedds(dhtmlObject.parentObject);
	else if(dhtmlObject.checkstate == 0) {
		alert($I18N.TREE_PARTNER_FEEDS);
		return;
	}
} 
 ...
 if(this.parentObject.checkstate==1)this.treeNod._setSubChecked(false,this.parentObject);
	 ...
	 if(this.parentObject.parentObject.id != this.treeNod.rootId && this.parentObject.partnerfeeds == 1)
		 this.treeNod._alertPartnerFedds(this.parentObject);
	 else if(this.parentObject.parentObject.id == this.treeNod.rootId && this.parentObject.partnerfeeds == 1)
		 alert($I18N.TREE_PARTNER_FEEDS);
 }
 else
 ...
 for(var i=0;i<itemObject.childsCount;i++)
{
 ...
 dhtmlXTreeObject.prototype.insertNewNext=function(parentItemId,itemId,itemName,itemActionHandler,image1,image2,image3,optionStr,childs,partnerfeeds){
 var sNode=this._globalIdStorageFind(parentItemId);
 if(!sNode)return(0);
 this._attachChildNode(0,itemId,itemName,itemActionHandler,image1,image2,image3,optionStr,childs,sNode,partnerfeeds);
};
