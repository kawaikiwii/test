    ...
    function get($id, $group = 'default', $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_caching) {
            ...
            if ($doNotTestCacheValidity) {
                ...
                $refreshTime = time() - $this->_lifeTime;
                if (file_exists($this->_file) && @filemtime($this->_file) > $refreshTime) {
                    $data = $this->_read();
                }
            }
    ...
    function remove($id, $group = 'default')
    {
        ...
        if (file_exists($this->_file)) {
            if (!@unlink($this->_file)) {
                $this->raiseError('Cache_Lite : Unable to remove cache ' . $id . '!', -3);   
                return false;
            }
        }
        return true;
