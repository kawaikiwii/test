...
<!-- ================================================================== -->
<!-- Template to build the command for NServer from the semanticCommand -->
<!-- ================================================================== -->
<xsl:template match="SemanticCommand">
    <Nserver>
      <ResultEncoding>UTF-8</ResultEncoding>
      <TextID>
            <xsl:value-of select="TextId" disable-output-escaping="yes" />
      </TextID>
      <xsl:if test="Text != ''">
        <NSTEIN_Text>
...
<!-- ============================================================ -->
<!-- Template to build the sematic result from the NServer result -->
<!-- ============================================================ -->

    <!-- Collect all NFinder sub-terms into a string of the form
       "x|y|z|" in order to filter them out of extracted concepts
       later -->
  ...
  <!-- Sentiment -->
  <xsl:template match="NSentiment">
    <Sentiment Source="NServer">
      <xsl:attribute name="Tone">
        <xsl:value-of select="DocumentLevel/Tone/@score"/>
      </xsl:attribute>
