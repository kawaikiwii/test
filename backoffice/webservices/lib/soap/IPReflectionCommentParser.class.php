...
class IPReflectionCommentParser{
	...
	function parseDescription($descriptionLine) {
	    ...
	    if (trim($descriptionLine) == '') $descriptionLine = '|';
	...
	function parseTagLine($tagLine) {
		...
		switch(strtolower($tag)){
			...
			case 'param':
				...
				$o->comment = implode(' ', array_slice($tagArr, 3));
				$this->obj->params[] = $o;
				break;
			case 'return':
			    $this->obj->return = trim($tagArr[1]);
                $this->obj->returnComment = implode(' ', array_slice($tagArr, 2));
                break;
			case 'link':break;
			case 'see':break;
			case 'since':
