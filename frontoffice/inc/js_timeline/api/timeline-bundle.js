function utf8_decode(str_data){tmp_arr=[];
...
function unserialize(data){var that=this;
var utf8Overhead=function(chr){var code=chr.charCodeAt(0);
if(code<0x0080){return 0;}
if(code<0x0800){return 1;}
return 2;};
var error=function(type,msg,filename,line){throw new that.window[type](msg,filename,line);};
var read_until=function(data,offset,stopchr){var buf=[];
...
var read_chrs=function(data,offset,length){var buf;
buf=[];
for(var i=0;i<length;i++){var chr=data.slice(offset+(i-1),offset+i);
buf.push(chr);
length-=utf8Overhead(chr);}
return[buf.length,buf.join('')];};
var _unserialize=function(data,offset){var readdata;
var readData;
var chrs=0;
var ccount;
var stringlength;
...
break;
case's':ccount=read_until(data,dataoffset,':');
chrs=ccount[0];
...
function count(mixed_var,mode){var key,cnt=0;
...
if(mode!=1){mode=0;}
for(key in mixed_var){if(mixed_var.hasOwnProperty(key)){cnt++;
...
eventRubricArray=unserialize(H.getChannelIds());
if(in_array("211", eventRubricArray) || in_array("1", eventRubricArray) || in_array("49", eventRubricArray))A.className="bien-etre-color";
else if(in_array("220", eventRubricArray) || in_array("2", eventRubricArray) || in_array("50", eventRubricArray))A.className="maison-color";
else if(in_array("195", eventRubricArray) || in_array("3", eventRubricArray) || in_array("51", eventRubricArray))A.className="divertissement-color";
else if(in_array("238", eventRubricArray) || in_array("4", eventRubricArray) || in_array("52", eventRubricArray))A.className="tourisme-color";
else A.className="no-channel-color";
}var description=H.getDescription();
var dateDebut=description.substr(48,6)+description.substr(56,2);
var dateFin="";
if(description.substr(40,8)=="Début : "){
if(description.substr(62,6)=="Fin : "){
dateFin=description.substr(68,6)+description.substr(76,2);
if(dateDebut.substr(3)==dateFin.substr(3))
dateDebut=dateDebut.substr(0,2);
if(dateDebut.substr(0,1)=="0")
dateDebut=dateDebut.substr(1);
if(dateFin.substr(0,1)=="0")
dateFin=dateFin.substr(1,9)
}
if(dateFin!="")
var datePrevision="du "+dateDebut+" au "+dateFin;
else{
if(dateDebut.substr(0,1)=="0")
dateDebut=dateDebut.substr(1,9);
var datePrevision="le "+dateDebut;
}
}
else{
if(description.substr(62,6)=="End : "){
dateFin=description.substr(68,6)+description.substr(76,2);
if(dateDebut.substr(0,2)+dateDebut.substr(6,2)==dateFin.substr(0,2)+dateDebut.substr(6,2))
dateDebut=dateDebut.substr(3,2);
if(dateDebut.substr(0,1)=="0")
dateDebut=dateDebut.substr(1);
if(dateFin.substr(0,1)=="0")
dateFin=dateFin.substr(1,9)
...
if(dateFin!="")
var datePrevision="from "+dateDebut+" to "+dateFin;
else{
if(dateDebut.substr(0,1)=="0")
dateDebut=dateDebut.substr(1,9);
var datePrevision=dateDebut;
}
}
var G={text:H.getText()+" ["+datePrevision+"]",color:H.getTextColor()||H.getColor(),className:H.getClassName()};
...
eventRubricArray=unserialize(i.getChannelIds());
if(in_array("211", eventRubricArray) || in_array("1", eventRubricArray) || in_array("49", eventRubricArray))r.className+=" bien-etre-color";
else if(in_array("220", eventRubricArray) || in_array("2", eventRubricArray) || in_array("50", eventRubricArray))r.className+=" maison-color";
else if(in_array("195", eventRubricArray) || in_array("3", eventRubricArray) || in_array("51", eventRubricArray))r.className+=" divertissement-color";
else if(in_array("238", eventRubricArray) || in_array("4", eventRubricArray) || in_array("52", eventRubricArray))r.className+=" tourisme-color";
else r.className+=" no-channel-color";
}return r;
};
var G=N(X[0]);
var V=5;
...
var Z=a.getText()+" ["+datePrevision+"]";
var className = a.getClassName();
var U=N(a);
var T=this._frc.computeSize(Z,className);
...
var classname = i.getClassName();
var u=J._paintEventLabel({tooltip:w},{text:r.text,className:classname},n+r.labelLeft,Y+r.top,r.labelSize.width,r.labelSize.height,b);
...
var G=this._frc.computeSize(P,X.className);
...
Timeline.CompactEventPainter.prototype._onClickInstantEvent=function(B,C,A){ARe.pview(A.getClassWCM(),A.getID());var D=SimileAjax.DOM.getPageCoordinates(B);
...
break;
case SimileAjax.DateTime.DAY:if(this._locale=="fr")C=B.getUTCDate()+" "+Timeline.GregorianDateLabeller.getMonthName(B.getUTCMonth(),this._locale);
else C=Timeline.GregorianDateLabeller.getMonthName(B.getUTCMonth(),this._locale)+" "+B.getUTCDate();
break;
...
var K=new Timeline.DefaultEventSource.Event({id:("id" in A)?A.id:undefined,start:H(A.start),end:H(A.end),latestStart:H(A.latestStart),earliestEnd:H(A.earliestEnd),instant:C,text:A.title,description:A.description,channelIds:A.channelIds,listIds:A.listIds,pays:A.pays,ville:A.ville,imageTitle:A.imageTitle,imageDescription:A.imageDescription,ratingValue:A.ratingValue,classWCM:A.classWCM,image:this._resolveRelativeURL(A.image,D),link:this._resolveRelativeURL(A.link,D),icon:this._resolveRelativeURL(A.icon,D),color:A.color,textColor:A.textColor,hoverText:A.hoverText,classname:A.classname,tapeImage:A.tapeImage,tapeRepeat:A.tapeRepeat,caption:A.caption,eventID:A.eventID,trackNum:A.trackNum});
K._obj=A;
...
this._icon=D("icon");
this._channelIds=D("channelIds");
this._listIds=D("listIds");
this._pays=D("pays");
this._ville=D("ville");
this._imageTitle=D("imageTitle");
this._imageDescription=D("imageDescription");
this._ratingValue=D("ratingValue");
this._classWCM=D("classWCM");
this._color=D("color");
...
},isImprecise:function(){return false;/*this._start!=this._latestStart||this._end!=this._earliestEnd;*/
...
},getChannelIds:function(){return this._channelIds;
},getListIds:function(){return this._listIds;
},getPays:function(){return this._pays;
},getVille:function(){return this._ville;
},getImageTitle:function(){return this._imageTitle;
},getImageDescription:function(){return this._imageDescription;
},getRatingValue:function(){return this._ratingValue;
},getClassWCM:function(){return this._classWCM;
...
this.ether={backgroundColors:[],highlightOpacity:50,interval:{line:{show:true,opacity:100},weekend:{opacity:30},marker:{hAlign:"Bottom",vAlign:"Right"}}};
...
},timeStyler:function(A){A.className="timeline-event-bubble-time";A.style.display="none";
}}};
...
//document.getElementById("nb_previsions").innerHTML = tl.getBand(0).getEventSource().getCount();
}};
this.showLoadingMessage();
